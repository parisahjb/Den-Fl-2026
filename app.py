...
# Fast version: zero LOO at startup — all metrics hardcoded from offline notebook
...
LOG_FEATS = [
    "Max_Acc", "Max_Vel", "Arias", "Char_Int", "CAV",
...
# ─────────────────────────────────────────────
# HARDCODED LOO METRICS (from offline notebook)
# ─────────────────────────────────────────────
METRICS = {
    "std": {
        "R2_t1": 0.733,  "RMSE_t1": 20.72, "MAE_t1": 13.13, "Bias_t1": -0.15, "Cov_t1": 89.3,
        "R2_t2": 0.738,  "RMSE_t2": 20.52, "MAE_t2": 12.57, "Bias_t2":  0.14, "Cov_t2": 89.3,
        "R2_t3": 0.721,  "RMSE_t3": 21.20, "MAE_t3": 14.50, "Bias_t3": -1.66, "Cov_t3": 89.3,
    },
    "log": {
        "R2_t1": 0.7533, "RMSE_t1": 19.9238, "MAE_t1": 12.2851, "Bias_t1": 1.3730, "Cov_t1": 89.33,
        "R2_t2": 0.7240, "RMSE_t2": 21.0721, "MAE_t2": 12.2429, "Bias_t2": 2.0559, "Cov_t2": 89.33,
        "R2_t3": None,   "RMSE_t3": None,    "MAE_t3": None,    "Bias_t3": None,   "Cov_t3": None,
    },
}

# Jackknife+ q values (90th percentile of |LOO residuals|)
Q_VALS = {
    "std": {"t1": 28.66, "t2": 28.71, "t3": 35.76},
    "log": {"t1": 0.3512, "t2": 0.3326, "t3": 0.3512},  # log-space q values
}

# Hardcoded lognormal LOO arrays (from offline notebook run)
LOO_T1_LOG = np.array([
    5.193392,4.794787,4.780479,4.520354,4.492114,4.836537,3.910919,4.952787,
    4.71655,4.936334,4.479352,4.081025,4.958566,4.73027,4.337847,4.345163,
    4.671018,4.416875,4.210397,4.074611,4.128159,4.126939,4.043965,4.448918,
    3.568544,4.173154,3.66072,4.270366,4.228967,4.060923,3.89887,3.502395,
    4.883089,3.999662,4.771796,3.980076,4.78199,3.903241,4.03852,3.815586,
    3.781854,3.86294,3.867462,4.131515,4.081206,4.140007,3.645348,4.460034,
    3.689036,3.752534,3.526989,4.12194,3.699514,4.424543,3.965907,3.891263,
    3.928144,3.476181,3.061044,3.988201,3.896353,3.185093,3.191553,3.639195,
    2.911212,3.925603,3.704576,3.839127,3.757402,3.312628,3.229046,2.889773,
    3.653958,4.490235,3.46326
])

LOO_T2_LOG = np.array([
    4.968948,4.631395,4.73096,4.592628,4.641902,4.881392,3.874254,4.82295,
    4.648974,5.000602,4.620215,4.127204,4.986555,4.806279,4.289357,4.296096,
    4.589504,4.429392,4.227564,4.035131,3.910914,3.938237,4.117824,4.449235,
    3.553535,4.206759,3.649148,4.225981,4.22516,4.154567,3.805802,3.526227,
    4.915289,4.050194,4.826493,3.983482,4.723849,3.991943,4.039589,3.821352,
    3.773518,3.829221,3.792068,4.129444,4.08269,4.184373,3.72202,4.530501,
    3.6864,3.83355,3.542271,4.102799,3.759629,4.352746,3.920625,3.848654,
    3.906062,3.502416,3.130189,3.908646,3.919534,3.342043,3.220498,3.658923,
    2.979994,3.837397,3.780246,3.795753,3.646063,3.325962,3.201677,2.866956,
    3.597897,4.442895,3.437564
])
...
def compute_all_kms(row):
    M_v=row["M"]; R_v=row["R"]; D=row["Max_Acc"]
    E=row["Max_Vel"]; F=row["Arias"]; K=row["SaT"]
    out = {}
    try:
    ...
    try:
        ratio=FC/D
        out["SR09"] = np.exp(-1.56-4.58*FC-20.84*ratio**2+44.75*ratio**3
                              -30.5*ratio**4-0.64*np.log(D)+1.55*np.log(E)
                              +0.405+0.524*ratio)
    except: out["SR09"] = 0.0
    try:
        ratio=FC/D
        out["TS16"] = np.exp(6.4-8.374*ratio-0.419*ratio**2+6.366*ratio**3
                              -7.031*ratio**4+0.767*np.log(D)+0.67*np.log(F/100))
    except: out["TS16"] = 0.0
    try:
        c1=6.31*Dr**0.06; c2=9.89*Dr**0.04; c3=1.27*Dr**0.05
        out["Lashgari_2021"] = np.exp(c1-c2*np.exp(np.log(FC/D)/c3)+np.log(FC*g))
    ...
    try:
        out["Youd_2002"] = 10**(-16.213+1.532*M_v
            ...
            +3.413*np.log10(100-0.7)-0.795*np.log10(0.18+0.1))*100
    except: out["Youd_2002"] = 0.0
    return out

def compute_song(ui):
    try:
        E=ui["Max_Vel"]; K=ui["SaT"]
        val = np.exp(-8.436-3.195*np.log(FC)-0.346*np.log(FC)**2
                     +1.579*np.log(K)+0.393*np.log(FC)*np.log(K)
                     -0.136*np.log(K)**2+1.542*np.log(E)+0.112*0.1)
...
class PhysicsInformedNN:
    def __init__(self, hidden_layers=(64,32,16), lr=0.0005, epochs=600,
                 batch_size=16, lambda_ts16=0.2, log_target=False, seed=42):
        self.hidden_layers=hidden_layers; self.lr=lr; self.epochs=epochs
        self.batch_size=batch_size; self.lambda_ts16=lambda_ts16
        self.log_target=log_target; self.seed=seed
        self.weights=[]; self.biases=[]; self.mu=None; self.sigma=None
    ...
    def _init_weights(self, n_in, rng):
        ...
        for i in range(len(sizes)-1):
            ...
            self.weights.append(rng.standard_normal((fi,sizes[i+1]))*np.sqrt(2.0/fi))
    ...
    def _backward(self, activations, y_true):
        ...
        delta=((2.0/n)*(y_pred-y_true)).reshape(-1,1)
        gw=[None]*len(self.weights); gb=[None]*len(self.biases)
        for i in range(len(self.weights)-1,-1,-1):
            a_in=activations[i]; a_out=activations[i+1]
            if i<len(self.weights)-1: delta=delta*(a_out>0)
            gw[i]=a_in.T@delta; gb[i]=delta.sum(axis=0,keepdims=True)
            delta=delta@self.weights[i].T
        return gw, gb

    def fit(self, X, y, X_raw_for_km=None):
        if X_raw_for_km is None: X_raw_for_km=X
        ...
        for ep in range(self.epochs):
            ...
            for start in range(0,n,self.batch_size):
                ...
                if self.lambda_ts16>0:
                    km_t=self._km_ts16(X_raw_for_km[b_idx]); nb=len(b_idx)
                    if self.log_target:
                        p_grad=(2.0*self.lambda_ts16/nb)*(y_pred-km_t)
                    else:
                        y_log=np.log1p(np.clip(y_pred,0,None))
                        p_grad=(2.0*self.lambda_ts16/nb)*(y_log-km_t)/(np.clip(y_pred,0,None)+1.0)
                    pd_=p_grad.reshape(-1,1)
                    pgw=[None]*len(self.weights); pgb=[None]*len(self.weights)
                    delta=pd_
                    for i in range(len(self.weights)-1,-1,-1):
                        a_in=acts[i]; a_out=acts[i+1]
                        if i<len(self.weights)-1: delta=delta*(a_out>0)
                        pgw[i]=a_in.T@delta; pgb[i]=delta.sum(axis=0,keepdims=True)
                        delta=delta@self.weights[i].T
    ...
    def predict(self, X):
        ...
        if self.log_target: return np.expm1(np.clip(pred,-10,20))
        else: return np.clip(pred,0,None)
...
# ─────────────────────────────────────────────
# STARTUP — fit full-data models only, NO LOO
# ─────────────────────────────────────────────
@st.cache_resource(show_spinner="Loading data and fitting models (~20 sec)...")
def load_and_train():
    ...
    df = (df_raw.rename(columns=col_map).iloc[:,:15]
               .dropna(subset=["Displacement"]).reset_index(drop=True))
    ...
    # ── Fit full-data models (no LOO)
    gb_std = GradientBoostingRegressor(
        n_estimators=200, max_depth=3, learning_rate=0.05,
        subsample=0.8, random_state=42).fit(X_t1_std, y)

    xgb_std = XGBRegressor(
        n_estimators=200, max_depth=4, learning_rate=0.05,
        subsample=0.8, colsample_bytree=0.8,
        tree_method="hist", random_state=42, verbosity=0
    ).fit(pd.DataFrame(X_t2_std, columns=T2_FEATS), y)

    gb_log = GradientBoostingRegressor(
        n_estimators=200, max_depth=3, learning_rate=0.05,
        subsample=0.8, random_state=42).fit(X_t1_log, y_log)

    xgb_log = XGBRegressor(
        n_estimators=200, max_depth=4, learning_rate=0.05,
        subsample=0.8, colsample_bytree=0.8,
        tree_method="hist", random_state=42, verbosity=0
    ).fit(pd.DataFrame(X_t2_log, columns=T2_FEATS), y_log)

    pinn_std = PhysicsInformedNN(
        lambda_ts16=0.2, log_target=False, epochs=600).fit(X_raw, y)

    pinn_log = PhysicsInformedNN(
        lambda_ts16=0.2, log_target=True, epochs=600).fit(
        X_raw_log, y_log, X_raw_for_km=X_raw)

    # SHAP (TreeExplainer — fast, no LOO needed)
    sv_std = shap.TreeExplainer(gb_std).shap_values(
        pd.DataFrame(X_t1_std, columns=TOP6))
    sv_log = shap.TreeExplainer(gb_log).shap_values(
        pd.DataFrame(X_t1_log, columns=TOP6))
    ...
    return {
        ...
        "std": {
            ...
            "sv": sv_std, "xgb_imp": xgb_std.feature_importances_,
        },
        "log": {
            "gb": gb_log, "xgb": xgb_log, "pinn": pinn_log,
            "X_t1": X_t1_log, "X_t2": X_t2_log,
            "sv": sv_log, "xgb_imp": xgb_log.feature_importances_,
        },
    }
...
# PREDICTION HELPER
# ─────────────────────────────────────────────
def predict_all_tiers(ui, data, use_log):
    md       = data["log"] if use_log else data["std"]
    song_val = compute_song(ui)
    song_log = np.log1p(song_val)
    x_raw    = np.array([[ui[f] for f in RAW_FEATURES]])
    x_t1_raw = np.array([[ui[f] for f in TOP6]])

    mode = "log" if use_log else "std"
    Q    = Q_VALS[mode]

    if use_log:
        x_t1     = log_transform_X(x_t1_raw, TOP6)
        x_t2     = np.column_stack([log_transform_X(x_t1_raw, TOP6), [[song_log]]])
        x_raw_in = log_transform_X(x_raw, RAW_FEATURES)
        ...
        p1, p2, p3 = float(np.expm1(lp1)), float(np.expm1(lp2)), float(np.expm1(lp3))

        pi1 = (float(np.expm1(max(-10, lp1-Q["t1"]))), float(np.expm1(lp1+Q["t1"])))
        pi2 = (float(np.expm1(max(-10, lp2-Q["t2"]))), float(np.expm1(lp2+Q["t2"])))
        pi3 = (float(np.expm1(max(-10, lp3-Q["t3"]))), float(np.expm1(lp3+Q["t3"])))
    else:
        x_t2 = np.array([[ui[f] for f in TOP6] + [song_log]])
        p1 = float(md["gb"].predict(x_t1_raw)[0])
        p2 = float(md["xgb"].predict(pd.DataFrame(x_t2, columns=T2_FEATS))[0])
        p3 = float(md["pinn"].predict(x_raw)[0])
        pi1 = (max(0.0, p1-Q["t1"]), p1+Q["t1"])
        pi2 = (max(0.0, p2-Q["t2"]), p2+Q["t2"])
        pi3 = (max(0.0, p3-Q["t3"]), p3+Q["t3"])
...
page = st.sidebar.radio("Go to", [
    ...
    "Model Comparison", "Explainability", "About"
])

st.sidebar.markdown("---")
st.sidebar.subheader("Preprocessing Mode")
preprocess_mode = st.sidebar.radio(
    "Select preprocessing", ["Standard", "Lognormal"],
    help=(
        "Standard: z-score normalization, linear target, symmetric PI.\n\n"
        "Lognormal: log1p-transform on right-skewed inputs + target. "
        "Asymmetric PI (wider on upper end). "
        "Follows geotechnical convention."
    )
)
...
y    = data["y"]
md   = data["log"] if use_log else data["std"]
met  = METRICS["log"] if use_log else METRICS["std"]
Q    = Q_VALS["log"] if use_log else Q_VALS["std"]

st.sidebar.markdown("---")
st.sidebar.markdown("**LOO-CV R² (offline)**")
...
st.sidebar.caption(f"Mode: **{preprocess_mode}** | n=75 | 90% Jackknife+ PI")
...
if page == "Single Prediction":
    ...
    st.markdown(
        ...
        "Enter 13 seismic intensity measures. All three tiers run simultaneously."
    )
    if use_log:
        st.info(
    ...
    if st.button("Run Prediction", type="primary"):
        ...
        for col, label, pred, pi, note in [
            (c1,"Tier 1: GB Top 6",       p1, pi1,"Baseline ML — 6 SHAP features"),
            (c2,"Tier 2: XGB + Song_2015", p2, pi2,"Knowledge-Augmented"),
            (c3,"Tier 3: PINN TS16 λ=0.2",p3, pi3,"Physics-Informed NN"),
        ]:
            col.markdown(f"**{label}**")
            col.metric("Predicted Displacement", f"{max(0,pred):.1f} cm",
        ...
        bars   = ax.bar(labels, preds, color=colors, alpha=0.85, edgecolor="white", width=0.5)
        ...
        st.info(f"Song_2015 auto-computed: {song_val:.3f} cm → "
                f"log1p = {np.log1p(song_val):.4f} (Tier 2 input)")

        with st.expander("Show all 7 empirical formula outputs"):
            kms   = compute_all_kms(pd.Series(user_inputs))
            km_df = pd.DataFrame({
                "Empirical Model": list(kms.keys()),
                "Predicted (cm)":  [f"{v:.2f}" for v in kms.values()],
                "Role": ["Tier 2 feature","—","—","—","Tier 3 physics loss","—","—"],
            })
            st.dataframe(km_df, use_container_width=True, hide_index=True)
            st.caption("Standalone best empirical R² = −7.18. "
                       "Formulas provide complementary signal inside ML.")
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"**Preprocessing: {preprocess_mode}** — "
                "Upload CSV or Excel. All three tiers run on every row.")

    with st.expander("Required column names"):
        st.code(", ".join(RAW_FEATURES))
        st.download_button(
            "Download blank template (CSV)",
            data=pd.DataFrame([FEATURE_DEFAULTS]).to_csv(index=False),
            file_name="seismic_batch_template.csv", mime="text/csv"
        )
    ...
    if uploaded is not None:
        ...
        if st.button("Run Batch Prediction", type="primary"):
            with st.spinner("Running predictions..."):
                ...
                for i, row in df_up.iterrows():
                    ...
                    for id_col in ["Rec_No","ID","id","Name"]:
                        if id_col in df_up.columns: rec[id_col] = row[id_col]
                    rec.update({
            ...
            ax.set_title(f"Batch Prediction Distribution — {preprocess_mode}", fontweight="bold")
            ...
            c1, c2 = st.columns(2)
            c1.download_button("Download as CSV",
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                ...
                pd.DataFrame({
                    "Model":         ["Tier 1: GB Top 6","Tier 2: XGB+Song_2015","Tier 3: PINN TS16 λ=0.2"],
                    "Preprocessing": [preprocess_mode]*3,
                    "LOO-CV R²":     [met["R2_t1"], met["R2_t2"],
                                      met["R2_t3"] if met["R2_t3"] else "pending"],
                    "LOO-CV RMSE":   [met["RMSE_t1"], met["RMSE_t2"],
                                      met["RMSE_t3"] if met["RMSE_t3"] else "pending"],
                    "PI type":       [pi_note]*3,
                }).to_excel(writer, index=False, sheet_name="Model Summary")
            c2.download_button("Download as Excel", data=buf.getvalue(),
                file_name=f"seismic_predictions_{preprocess_mode.lower()}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            st.caption(f"90% Jackknife+ PI ({pi_note}). T1/T2/T3 = Tier 1/2/3.")
...
# ══════════════════════════════════════════════════════════════════
    ...
    tab_cur, tab_both = st.tabs([
        f"Current Mode ({preprocess_mode})", "Standard vs Lognormal"
    ])
    ...
    def scatter_plot(y_true, y_pred, title, color):
        ...
        ax.scatter(y_true, y_pred, alpha=0.75, s=40, color=color,
    ...
    with tab_cur:
        pi_w = "asymmetric" if use_log else None
        results_rows = [
            {"Tier":"Empirical","Model":"HL_2010 (best standalone)",
             "R²":-7.18,"RMSE (cm)":114.7,"MAE (cm)":75.3,"Coverage (%)":"—","PI Width (cm)":"—"},
            {"Tier":"Tier 1","Model":f"GB Top 6 [{preprocess_mode}] ★",
             "R²":met["R2_t1"],"RMSE (cm)":met["RMSE_t1"],"MAE (cm)":met["MAE_t1"],
             "Coverage (%)":met["Cov_t1"],
             "PI Width (cm)": pi_w if use_log else round(2*Q["t1"],1)},
            {"Tier":"Tier 2","Model":f"XGB + Song_2015 [{preprocess_mode}] ★",
             "R²":met["R2_t2"],"RMSE (cm)":met["RMSE_t2"],"MAE (cm)":met["MAE_t2"],
             "Coverage (%)":met["Cov_t2"],
             "PI Width (cm)": pi_w if use_log else round(2*Q["t2"],1)},
            {"Tier":"Tier 3","Model":f"PINN TS16 λ=0.2 [{preprocess_mode}]",
             ...
             "PI Width (cm)": pi_w if use_log else round(2*Q["t3"],1)},
        ]
        ...
        if use_log:
            # Lognormal: scatter available immediately from hardcoded arrays
            c1, c2 = st.columns(2)
            with c1:
                fig = scatter_plot(y, LOO_T1_LOG_CM,
                                   "Tier 1: GB Top 6 [Lognormal]", "#1565C0")
                st.pyplot(fig); plt.close()
            with c2:
                fig = scatter_plot(y, LOO_T2_LOG_CM,
                                   "Tier 2: XGB+Song [Lognormal]", "#BF360C")
                st.pyplot(fig); plt.close()
        else:
            # Standard: compute on demand to avoid slow startup
            st.info(
                "LOO scatter for Standard mode requires ~30 sec to compute "
                "(75 model fits per tier). Click the button below when ready."
            )
            if st.button("Compute Standard LOO Scatter (~30 sec)"):
                with st.spinner("Running LOO-CV for scatter plots..."):
                    loo_t1_std = np.zeros(len(y))
                    for tr, te in LeaveOneOut().split(data["std"]["X_t1"]):
                        m = GradientBoostingRegressor(
                            n_estimators=200, max_depth=3,
                            learning_rate=0.05, subsample=0.8, random_state=42)
                        m.fit(data["std"]["X_t1"][tr], y[tr])
                        loo_t1_std[te[0]] = m.predict(data["std"]["X_t1"][te])[0]
                    loo_t2_std = np.zeros(len(y))
                    for tr, te in LeaveOneOut().split(data["std"]["X_t2"]):
                        m = XGBRegressor(
                            n_estimators=200, max_depth=4, learning_rate=0.05,
                            subsample=0.8, colsample_bytree=0.8,
                            tree_method="hist", random_state=42, verbosity=0)
                        m.fit(pd.DataFrame(data["std"]["X_t2"][tr], columns=T2_FEATS), y[tr])
                        loo_t2_std[te[0]] = m.predict(
                            pd.DataFrame(data["std"]["X_t2"][te], columns=T2_FEATS))[0]
                c1, c2 = st.columns(2)
                with c1:
                    fig = scatter_plot(y, loo_t1_std,
                                       "Tier 1: GB Top 6 [Standard]", "#2196F3")
                    st.pyplot(fig); plt.close()
                with c2:
                    fig = scatter_plot(y, loo_t2_std,
                                       "Tier 2: XGB+Song [Standard]", "#FF5722")
                    st.pyplot(fig); plt.close()

        st.subheader("Tier 3: PINN — LOO Metrics")
        if use_log:
            st.warning("PINN lognormal LOO metrics pending offline computation.")
        else:
            c1,c2,c3,c4 = st.columns(4)
            c1.metric("R²","0.721"); c2.metric("RMSE","21.20 cm")
            c3.metric("MAE","14.50 cm"); c4.metric("Coverage","89.3%")
            st.caption("Standard PINN: offline notebook (75 LOO folds × 600 epochs).")

    with tab_both:
        st.subheader("Standard vs Lognormal — Direct Comparison")
        ms = METRICS["std"]; ml = METRICS["log"]
        comp_df = pd.DataFrame({
            "Model":          ["GB Top 6 (Tier 1)","XGB+Song_2015 (Tier 2)"],
            "R² — Standard":  [ms["R2_t1"], ms["R2_t2"]],
            "R² — Lognormal": [ml["R2_t1"], ml["R2_t2"]],
            "RMSE — Standard":[ms["RMSE_t1"], ms["RMSE_t2"]],
            "RMSE — Lognormal":[ml["RMSE_t1"], ml["RMSE_t2"]],
            "MAE — Standard": [ms["MAE_t1"], ms["MAE_t2"]],
            "MAE — Lognormal":[ml["MAE_t1"], ml["MAE_t2"]],
        })
        st.dataframe(comp_df, use_container_width=True, hide_index=True)
        st.caption("All metrics from LOO-CV. Lognormal metrics on back-transformed cm predictions.")

        fig, ax = plt.subplots(figsize=(8,4))
        x=np.arange(2); w=0.3
        b1=ax.bar(x-w/2,[ms["RMSE_t1"],ms["RMSE_t2"]],w,label="Standard",
                  color="#1976D2",alpha=0.85)
        b2=ax.bar(x+w/2,[ml["RMSE_t1"],ml["RMSE_t2"]],w,label="Lognormal",
        ...
        # Lognormal scatters always available; standard via button
        st.markdown("---")
        st.subheader("LOO Scatter — Lognormal Mode")
        c1,c2=st.columns(2)
        with c1:
            fig=scatter_plot(y,LOO_T1_LOG_CM,"Tier 1 — Lognormal","#1565C0")
            st.pyplot(fig); plt.close()
        with c2:
            fig=scatter_plot(y,LOO_T2_LOG_CM,"Tier 2 — Lognormal","#BF360C")
            st.pyplot(fig); plt.close()
...
# ══════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        if use_log:
            st.info("SHAP values on log1p-transformed features. "
        ...
        st.subheader("Consensus SHAP Rankings (All 13 features)")
    ...
    with tab2:
        ...
        st.caption("✓ = improvement vs Top 6 baseline (Standard mode results).")

    with tab3:
        st.subheader(f"CAV SHAP Dependence [{preprocess_mode}]")
        cav_idx = TOP6.index("CAV")
        fig,ax  = plt.subplots(figsize=(9,5))
        ax.scatter(X_t1[:,cav_idx], sv[:,cav_idx], alpha=0.8, s=55,
                   color="#2196F3", edgecolors="white", linewidths=0.5)
        ...
        ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
        plt.tight_layout(); st.pyplot(fig); plt.close()
        st.markdown("CAV is the dominant predictor across all models. High CAV drives "
                    "large positive SHAP contributions — reflecting the physical role "
                    "of cumulative seismic energy in triggering pore-pressure buildup.")
...
# ══════════════════════════════════════════════════════════════════
    ...
    Predicts **lateral displacement (cm)** from 13 seismic intensity measures.
    Trained on n=75 numerical simulation records.
    Evaluated with **LOO-CV**. Uncertainty via **Jackknife+ PI** at 90% nominal coverage.
    """)
    st.markdown("---")
    ...
    **Standard**
    ...
    - Linear target (cm)
    - Symmetric Jackknife+ PI (±q cm)
    """)
    c2.markdown("""
    **Lognormal**
    - log1p-transform: CAV, Arias, Housner, Max_Vel, Max_Acc, Char_Int, Sa_avg, SaT, Ia_Du, Shear_strain
    - M, R, ru_max_ave kept as-is
    - Target: log1p(displacement) → back-transformed to cm
    - Asymmetric PI — wider on upper end
    - Follows geotechnical convention
    """)
    st.markdown("---")
    st.subheader("LOO-CV Performance Summary")
    summary_df = pd.DataFrame({
        "Model":           ["GB Top 6","XGB+Song_2015","PINN TS16 λ=0.2"],
        "R² — Standard":   [0.733, 0.738, 0.721],
        "RMSE — Standard": [20.72, 20.52, 21.20],
        "R² — Lognormal":  [0.7533, 0.7240, "pending"],
        "RMSE — Lognormal":[19.92, 21.07, "pending"],
    })
    ...
    **Technical notes:**
    - Startup fits 6 models on full data only (~15–20 sec). No LOO at startup.
    - All LOO metrics hardcoded from offline notebook runs.
    - Lognormal LOO arrays hardcoded for instant scatter plots.
